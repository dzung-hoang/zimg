...
std::vector<ColorspaceNode> get_neighboring_colorspaces(const ColorspaceDefinition &csp)
{
	...
	if (csp.matrix == MatrixCoefficients::RGB) {
		...
		for (auto coeffs : all_matrix()) {
			if (coeffs != MatrixCoefficients::RGB && coeffs != MatrixCoefficients::REC_2020_CL &&coeffs != MatrixCoefficients::REC_2020_ICTCP && 
			    coeffs != MatrixCoefficients::REC_2020_LMS && coeffs != MatrixCoefficients::UNSPECIFIED) {
		...
		// Linear RGB can also be converted to REC_2020_CL and REC_2100_LMS.
